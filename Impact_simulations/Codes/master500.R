...
auxPath <- path.expand("~")
if(FIT){
    if(!dir.exists(paste0(auxPath, "/TB_SIM_OUT_Bien_KCaR_TrNorm/", country, '/Object_Folder/Iter_', sprintf("%03d", 1), '/'))){
        recorrido <- IterMin:IterMax
    } else{
        out <- c(0)
        j = 1
        for (i in start_to_end){
            path_check <- paste0(auxPath, "/TB_SIM_OUT_Bien_KCaR_TrNorm/", country, '/Object_Folder/Iter_', sprintf("%03d", i), '/Output_object_1.txt')
        ...
        while(handler==0){
            aux <- tryCatch(
            {
                ...
                if(FIT){
                    create_recal_parameters(country, i, i)
                    create_recal_contacts(country, i, i, region)
                    create_initial_fit_param(country, i)
                    system(paste0("cp ../Outputs/", country, "/Fitted_inputs/Initial_Conditions/initial_conditions.txt ../Outputs/", country, "/Iter/", country, "_", sprintf("%03d", i), "/Fitted_inputs/Initial_Conditions/initial_conditions.txt"))
                }
