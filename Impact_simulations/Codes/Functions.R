...
set_TBsim_workspace <- function(ctr, itr){
    ...
    if(itr==0){ 
        ...
        #system(paste0("cp ../Outputs/", ctr, "/Fitted_inputs/Initial_Conditions/initial_conditions_Harris_arch.txt ../Outputs/", ctr, "/Fitted_inputs/Initial_Conditions/initial_conditions.txt"))
    }
    ...
    if(itr>0){
        ...
        if (!dir.exists(output_dir)){
            ...
            if(!file.exists(destfile)){
                ...
                system(paste0("cp ../Outputs/", ctr, "/Fitted_inputs/Parameters_to_fit.txt ../Outputs/", ctr, "/Iter/", ctr, "_", sprintf("%03d", itr), "/Fitted_inputs/Parameters_to_fit.txt"))
            }
...
create_maste_file <- function(ctr, itr, reg){
    ...
    if(itr==0){
        ...
        if(reg==1){
            aux <- sprintf("../Inputs/Contacts/Ref_matrix.txt") #sprintf(aux, "../Inputs/Contacts/Asia_contacts_pi.txt")
        }
        ...
        if(reg==1){
            aux <- sprintf("../Inputs/Contacts/Ref_matrix.txt") 
            #aux <- paste0("../Inputs_Iter/", ctr,"/Contacts/Asia_contacts_pi_", sprintf("%03d", itr), ".txt")
        }
...
#' @keywords Error
create_recal_parameters <- function(country, ini, fin){
    ...
    for(a in ini:fin){
        ...
        #new <- rnorm(length(mean), mean, sd)
        new <- no_negative_distribution(mean, par$V8, par$V9)
        par$V7 <- new
    ...
    for(a in ini:fin){
        ...
        #new <- rnorm(length(mean), mean, sd)
        new <- no_negative_distribution(mean, reg$V8, reg$V9)
        reg$V7 <- new
...
no_negative_distribution <- function(mu, alpha, beta){
    if(length(mu) == 1){
        if(alpha > beta){
            b <- log(alpha)
            a <- log(beta)
            sigma = (alpha - beta)/3.92
        } else{
            a <- log(alpha)
            b <- log(beta)
            sigma = (beta - alpha)/3.92
        }
        if(mu==0){
            return(mu)
        } else{
...
fitter <- function(ctr, itr, fit_span=19, mod_cts=0, MFILE, func_fitness = fitness_INC_MORT, ...) {
    ...
    auxPath <- path.expand("~")
    if(itr==0){
        pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctr, '/Object_Folder/Baseline')
    } else{
        pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctr, '/Object_Folder/Iter_', sprintf("%03d", itr))
    }
...
simTB_iter <- function(ctry, iteration=0, mode_contact=3, fitting=T, running=T, running_vaccine=F, FuncFitness = fitness_INC_MORT, ...){
    ...
    if(fitting){
        ...
        if(require(minpack.lm)){
            ...
            auxPath <- path.expand("~")
            if(iteration==0){
                weightPathOut <- paste0(auxPath, "/TB_SIM_OUT/", ctry, '/Object_Folder/Baseline/poids_', ctry, ".txt")
            } else{
                weightPathOut <- paste0(auxPath, "/TB_SIM_OUT/", ctry, '/Object_Folder/Iter_', sprintf("%03d", iteration), '/poids_', ctry, ".txt")
            }
    ...
    if(running){
        ...
        auxPath <- path.expand("~")
        if(iteration==0){
            pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctry, '/Object_Folder/Baseline')
        } else{
            pathout <- paste0(auxPath, "/TB_SIM_OUT/", ctry, '/Object_Folder/Iter_', sprintf("%03d", iteration))
        }
        if (!dir.exists(pathout)){
            dir.create(pathout, recursive = TRUE)
        }
        ...
        if(save_vaccine && running_vaccine){
            ...
            if(iteration==0){
                pathout_vaccine <- paste0(auxPath, "/TB_SIM_OUT/", ctry, "/Vaccines/Vac_", 
