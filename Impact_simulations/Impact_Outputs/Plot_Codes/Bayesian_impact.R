...
for (country in countries){
    IRR_M <- list()
    for(model in 1:7){
        df_aux <- read.table(paste0('../iteration_impacts/', country, '_M', model, '_dataimp.txt'), header=T)
...
write.table(df_out, "Model_Combined_IRR.txt", quote=FALSE, row.names=FALSE)
